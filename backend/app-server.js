...
// -----------------------------------------------------------------------------
// mongoose.connect('mongodb://${process.env.SF_USERNAME}:${process.env.SF_PASSW
// ORD}@${process.env.SF_URL}:${process.env.SF_PORT}/${process.env.SF_DB_NAME}')
// ; Tell any CSS tooling (such as Material UI) to use all vendor prefixes if
// the user agent is not known.
// -----------------------------------------------------------------------------
...
// Routing
// -----------------------------------------------------------------------------
// since this file is not es6 compatible, when importing files with es6 code
// that export default -- call the default method
const appRouter = require('../src/routes');
const renderToString = reactDomServer.renderToString;
const routes = createRoutes(appRouter.default());
// array of static files that will be requested by the browser
const staticFiles = ['/static/*', '/images/*', '/asset/manifest.json', '/favicon.ico'];
...
app.get('*', (req, res) => {
    ...
    fs.readFile(indexPath, 'utf-8', (err, htmlData) => {
        if (err) {
            ...
            // browserHistory does not work with node, we simulate it with the match()
            // method
            match({
                routes,
                location: req.url
