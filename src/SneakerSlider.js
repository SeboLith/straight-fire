...
class SneakerSlider extends React.Component {
  ...
  getSneakerImages() {
    ...
    const apiRoute = (window.location.host === 'localhost:3000' || window.location.host === 'localhost:8090')
  ...
  render() {
    ...
    const kicks = this
      ...
      .map((filePath, index) => {
        ...
        let image = <div key={String(index)}>
          <img key={String(index)} src={filePath} alt={filePath} ref="simpleSquare"></img>
        </div>;
        return image;
      });
    ...
    return (
      <div className="slider-container">
        {this.state.filePaths.length && <Slider {...settings}>
          {kicks}
        </Slider>}
      </div>
    );
